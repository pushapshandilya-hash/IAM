...
- [Appendix A. Approved Technologies](#appendix-a-approved-technologies)
...
This standard defines the mandatory security controls for all service mesh implementations (e.g. Istio, Linkerd, Consul Connect, AWS App Mesh, Kuma, Open Service Mesh). It is mesh-agnostic and applies to both sidecar and sidecar-less (ambient) data planes.
...
This document uses RFC 2119 / RFC 8174 keywords. **MUST** denotes a mandatory requirement; **SHOULD** denotes a recommended requirement that requires a documented exception to omit; **MAY** denotes an optional measure.

| Term | Definition |
|---|---|
| **Control Plane** | Mesh components that configure and manage policy, identity, and certificates (e.g. istiod). |
| **Data Plane** | The proxies (sidecar or ambient) that enforce policy and carry workload traffic. |
| **Workload Identity** | A unique cryptographic identity bound to a workload, SPIFFE-aligned (e.g. an SVID). |
| **Trust Domain** | The administrative boundary within which workload identities share a common root of trust. |
| **mTLS Mode** | Mesh TLS posture: Permissive (plaintext + mTLS accepted) or Strict (mTLS only). |
| **East-West Traffic** | Service-to-service traffic within or between clusters, as opposed to ingress / egress. |
...
| **SM-AUTHZ-04** | Administrative access MUST be separated from operational access (mesh RBAC + Kubernetes RBAC). | High |
...
| **SM-MTLS-03** | Plaintext service traffic MUST NOT exist in production; gateway traffic MUST use TLS. | Critical |
...
| ID | Control Requirement | Crit. |
|---|---|---|
| **SM-CERT-01** | Certificates MUST be issued only from an approved Certificate Authority ([Appendix A](#appendix-a-approved-technologies)). | Critical |
...
| **SM-CFG-02** | Data-plane proxies MUST enforce automatic injection, run patched images, and set resource limits. | High |
| **SM-CFG-03** | Gateways MUST use TLS 1.2+ with approved cipher suites, strong certificate validation, and access logging. | High |
...
| **SM-VULN-02** | Vulnerabilities MUST be remediated within SLA: Critical 7 days, High 30 days, Medium 90 days. | High |
...
| Policy-as-Code | GitOps + admission enforcement | SM-PAC-01/03 |
...
Domain-level coverage across regulatory frameworks. Control-level NIST 800-53 / CSF traceability is provided in [Appendix B](#appendix-b-control-to-framework-reference).

| Control Domain | Priority | PCI-DSS | SOC 2 | ISO 27001 | HIPAA | NIST |
|---|---|:---:|:---:|:---:|:---:|:---:|
| Workload Identity (SM-IAM) | Critical | ✓ | ✓ | ✓ | ✓ | ✓ |
| Authorization (SM-AUTHZ) | Critical | ✓ | ✓ | ✓ | ✓ | ✓ |
| mTLS (SM-MTLS) | Critical | ✓ | ✓ | ✓ | ✓ | ✓ |
| Certificate Mgmt (SM-CERT) | Critical | ✓ | ✓ | ✓ | ✓ | ✓ |
| Secrets Mgmt (SM-SEC) | Critical | ✓ | ✓ | ✓ | ✓ | ✓ |
| Network Segmentation (SM-NET) | High | ✓ | ✓ | ✓ | ✓ | ✓ |
| Egress Control (SM-EGR) | High | ✓ | ✓ | ✓ | ✓ | ✓ |
| Logging / Monitoring (SM-LOG/MON) | High | ✓ | ✓ | ✓ | ✓ | ✓ |
| Policy-as-Code (SM-PAC) | High | ✓ | ✓ | ✓ | – | ✓ |
| Vulnerability Mgmt (SM-VULN) | High | ✓ | ✓ | ✓ | ✓ | ✓ |
...
## Appendix A. Approved Technologies

*Informative. Maintained by Security Architecture and updated independently of this standard's review cycle.*

| Control Area | Approved Technologies |
|---|---|
| Certificate authorities | Enterprise PKI; HashiCorp Vault; Cloud-provider managed CA; cert-manager |
| Secrets management | HashiCorp Vault; cloud secret managers; CSI Secret Store drivers |
| Federation protocols | OpenID Connect (OIDC); OAuth 2.0; SAML 2.0 |
| Policy enforcement | Admission controllers (e.g. OPA Gatekeeper, Kyverno); CI/CD policy validation |
...
|---|---|---|---|
| 2.0 | YYYY-MM-DD | Security Architecture | Security Governance Board |

...
**Control criticality** (the Crit. column) rates the importance of the control: **Critical** — failure blocks production authorization and requires immediate remediation; **High** — significant risk, remediated on a defined, tracked timeline; **Medium** — lower risk, remediated in the normal cycle. Control criticality is distinct from vulnerability severity in SM-VULN, which rates individual findings.

Where a control references an "approved" technology, authority, or list, approval is granted by Security Architecture and recorded in the applicable enterprise register (e.g., the enterprise cryptographic / PKI standard or [Appendix A](#appendix-a-approved-technology-categories)).
...
| **Acronyms** | MFA (multi-factor authentication); SIEM (security information and event management); RBAC (role-based access control); PKI (public-key infrastructure); HSM/KMS (hardware security module / key management service); mTLS (mutual TLS); NTP (network time protocol). |
